...
- **Concurrency:** cog runs 1 prediction at a time; two simultaneous pipelines (qwen 5 GB + whisper ~3.9 GB + 2.4 GB resident) would brush the 16 GB limit — rely on existing sequential bridge/redis locking
- **Timeouts:** redis worker timeouts (`REDIS_SOCKET_TIMEOUT=120`) vs 90-minute meetings — 36 min measured ≈ 2 min of transcription (~41.7 s), so a 90-minute meeting stays within budget, but verify against the deployed timeout settings
