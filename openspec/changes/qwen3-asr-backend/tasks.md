...
- [x] 5.3 Résolu : bake `/models` abandonné (bind mount hôte + `scripts/provision.py`), puis E5-LEGACY-HF : flags offline `HF_HUB_OFFLINE=1`/`TRANSFORMERS_OFFLINE=1` SUPPRIMÉS de `cog.yaml` (ils bloquaient le téléchargement pyannote gated au 1er run diarize) ; `local_files_only=True` et la porte fail-hard `E_MODEL_NOT_PROVISIONED` retirées du runtime — résolution : /models provisionné sinon repo id HF (téléchargement runtime), provision.py + models.lock restent en tooling CI
